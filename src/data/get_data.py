...
def apply_transformations(df, transformations):
    for transformation in transformations:
        if 'filter' in transformation:
            ...
            agg_exprs = [
                sum(op['sum']).alias('sum_' + op['sum']) if 'sum' in op else (
                    count(op['count']).alias('count_' + op['count']) if 'count' in op else ""
                )
    ...
    for table in config['tables']:
        if 'joins' in table:
            ...
            for join in table['joins']:
                ...
                join_condition = [col(key) == col(value) for key, value in join['join_condition'].items()]
