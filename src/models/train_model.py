...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=model_config['training']['test_split'])
...
if 'cross_validation' in model_config['training']:
    scores = cross_val_score(model, X, y, cv=model_config['training']['cross_validation']['k_folds'])
...
evaluation_results = evaluate_model(model, X_test, y_test, model_config['evaluation_metrics'])
